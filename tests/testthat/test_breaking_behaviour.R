...
test_that("errors for invalid variable names", {
...
test_that("errors and warnings for data problems", {
    ### Data problems
    ...
    # Unbalanced panel, do we get the message?
    unbalanced_data <- as.matrix(data[
        sample(1:nrow(data), floor(nrow(data) * 0.7)),
    ])
    # Get the message if we get positive data rows
    expect_message(
        dbc(
            outcome_fml = y ~ lag_y + D + X1_1,
            treatment_fml = D ~ lag_y + X2_1,
